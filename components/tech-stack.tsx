...
import { 
  Code, FileJson, FileType, CircleIcon, 
  Globe, Smartphone, Server, PaintBucket, 
  Wind, Database, Lock, Network, 
  GitBranch, Github, Vercel, TerminalSquare,
  MonitorCode
} from "lucide-react"
...
const technologies = [
  ...
  },
  {
    ...
    techs: ["Node.js", "Express.js", "Firebase", "MongoDB", "JWT Auth", "REST API"],
  },
  {
...
export default function TechStack() {
  ...
  return (
    <section id="tech-stack" className="py-20 md:py-28 bg-muted/30 min-h-screen">
      <div className="container px-4 md:px-6">
        <SectionHeading title="Tech Stack" subtitle="Technologies and tools I work with regularly." />
            ...
            >
              <Card className="h-full border border-primary/10 bg-background/50 backdrop-blur-sm hover:shadow-md hover:border-primary/20 transition-all duration-300">
                <CardContent className="p-4 sm:p-6">
                  <h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-primary">{tech.category}</h3>
                  <div className="flex flex-wrap gap-1.5 sm:gap-2.5">
                    {tech.techs.map((item) => (
                      <motion.div key={item} variants={itemVariants} className="transition-transform hover:-translate-y-1 duration-200">
                        <Badge 
                          variant="secondary" 
